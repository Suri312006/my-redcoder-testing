...
# TODO: need to change these things
# from transformers.modeling_bert import BertConfig, BertModel
# from transformers.optimization import Adam
...
def get_bert_biencoder_components(args, inference_only: bool = False, **kwargs):
    ...
    fix_ctx_encoder = args.fix_ctx_encoder if hasattr(
        args, 'fix_ctx_encoder') else False
...
class HFRobertaEncoder(RobertaModel):
    ...
    def init_encoder(cls, cfg_name: str, projection_dim: int = 0, dropout: float = 0.1, **kwargs) -> RobertaModel:
        cfg = RobertaConfig.from_pretrained(
            cfg_name if cfg_name else 'roberta-base')
        if dropout != 0:
            cfg.attention_probs_dropout_prob = dropout
            cfg.hidden_dropout_prob = dropout
        return cls.from_pretrained(cfg_name, config=cfg, project_dim=projection_dim, **kwargs)
    ...
    def forward(self, input_ids: T, token_type_ids: T, attention_mask: T) -> Tuple[T, ...]:
        ...
        #so the forward method is just returning strings
        # the new forward method might be returning something different?
        # print(f"Sequence_Output = {sequence_output}, type = {type(sequence_output)}")
        # print(f"pooled_output = {pooled_output}, type = {type(pooled_output)}")
        # print(f"hidden_states = {hidden_states}, type = {type(hidden_states)}")
